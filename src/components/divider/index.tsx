...
function getVariant(type: Variant, key: number): JSX.Element {
...
function Divider(): JSX.Element {
  ...
  const squareSizeWithMargin = 31.6; // Размер квадрата с учётом отступов
  ...
  return (
    <div ref={containerRef} className={styles.grid_container}>
      {Array(squaresPerRow * 2).fill(0).map((_, index) => (
        getVariant(getRandomElement(), index)
      ))}
    </div>
  )
}
