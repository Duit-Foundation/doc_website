...
Команды отправляются через [CommandEvent](../core_concepts/actions_events.md#commandevent) и обрабатываются контроллерами виджетов. Каждый тип команды имеет свою специализированную реализацию, которая наследуется от `RemoteCommand`.
