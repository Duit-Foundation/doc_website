...
Для действий `TransportAction` и `ScriptAction` Duit поддерживает механизм идемпотентного выполнения. При включённой идемпотентности повторные вызовы действия с идентичными входными данными (собранными из `dependsOn`) подавляются — запрос на сервер не отправляется.

Используется хеш-кэш: при каждом вызове вычисляется хеш данных из зависимостей. Если хеш совпадает с последним успешным выполнением, действие не выполняется.

Параметры:

- **`idempotent`** (bool) — включает режим идемпотентного выполнения. По умолчанию `false`.
- **`suppressionTTL`** (int, миллисекунды) — опционально. Время, по истечении которого разрешается повторное выполнение с теми же данными. Если не указан, подавление дубликатов действует бессрочно (пока данные не изменятся).

:::tip
Используйте идемпотентность для избежания лишних запросов при быстрых повторных нажатиях кнопок или при срабатывании нескольких виджетов с одинаковым действием и данными.
:::
...
В примере выше действие с идентичным текстом поиска не будет выполняться повторно в течение 5 секунд после последнего выполнения.
